...
var TxRequest = function (pType, wannaSample) {
    ...
    if (wannaSample === false) { // description
        ...
        var amount = new BigNumber(10);
        if (pType !== 1) {
            amount = 0;
        }
        var fee = new BigNumber(1000000000000000000);
        if (pType === 3) { // bonding
            fee = new BigNumber(10000000000000000000000);
        } else if (pType === 7) {
            fee = new BigNumber(10000000000000000000);
        } else if (pType === 8) {
            fee = new BigNumber(0);
        } else if (pType === 10){
            fee = new BigNumber(1000000000000000000000);
        }
...
var showDescription = function(type) {
    ...
    switch (type) {
        ...
        case 10: // MakeXChainType
            var v1 = new ValidatorType("pub_key_type1 (type: string)", "value1 (type: string)", "power1 (type: string)");
            var v2 = new ValidatorType("pub_key_type2 (type: string)", "value2 (type: string)", "power2 (type: string)");
            res = {
...
var showSample = function (type) {
    ...
    switch (type) {
        ...
        case 5: // DelegatingType
            res = {
                validatorAccountAddr: "9DE91697E8FA30A7424E229121782FC8CC62ABA0",
                amount: 50
            };
            break;
        case 6: // UndelegatingType
        ...
        case 7: // GRProposalType
            res = {
                rewardAmount:       1,
                validatorRewardRate:2,
                delegatorRewardRate:3,

                minCommonTxFee:     4,
                minBondingTxFee:    5,
                minGRProposalTxFee: 6,
                minGRVoteTxFee:     7,
                minXTxFee:          8,

                maxBlockNumsForVoting:      9,
                minBlockNumsToGRProposal:   10,
                blockNumsUtilReflection:    11,

                blockNumsFreezingValidator: 12,
                blockNumsUtilUnbonded:      13,
                maxDelegatableValidatorNums:14,
                validatorNums:              15,
