...
var errStrMap = {
    ...
    inputMustAtxtype3: "The currency type must be  "
};
...
var inputSendTransaction1 = function (param) {
    ...
    if (param.time != undefined && param.time != null) {
        param.time = objValueCheckStringNumber("Time", param.time);
    }
    // viewObj("111", param);
...
var inputMustAtxtype3 = function (param) {
    ...
    if (atxType == null) {
        throw new Error(errStrMap.inputMustAtxtype3 + utils.getCurrencyTypes());
    }
    return atxType;
};
...
function checkPayloadBody(pType, pBodyObj) {
    if (pType === 1) {
        ...
        pBodyObj.rewardXtoPerCoin = objValueCheckStringNumber("payloadBody.rewardXtoPerCoin", pBodyObj.rewardXtoPerCoin, pType);
