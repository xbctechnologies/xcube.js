...
var inputSendTransaction1 = function (param) {
    ...
    param.fee = objValueCheckNumToStr("Fee", param.fee);
    param.amount = objValueCheckNumToStr("Amount", param.amount);
...
function checkPayloadBody(pType, pBodyObj) {
    if (pType === 1) {
        ...
        objValueCheckNumber("payloadBody.amount", pBodyObj.amount);
    }
        ...
        for(var key in pBodyObj){
            if (key === "firstCompatibleVersion"){
                objValueCheckString("payloadBody." + key, pBodyObj[key]);
            } else {
                objValueCheckNumber("payloadBody." + key, pBodyObj[key]);
            }
        }
    }
    else if (pType === 8 ) {
