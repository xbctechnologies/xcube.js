...
var objValueCheckStringNumber = function (key, value) {
    if (Number(value) >= 0) {
        return value
    } else {
...
var inputSendTransaction1 = function (param) {
    ...
    // console.log("aaaaaa ", param.payloadBody.amount);
    ...
    // viewObj("inputSendTransaction1", param);
...
var outputPayloadBodyFormatter = function (res) {
    ...
    parseObj(res);
    /*res.amount = utils.toBigNumber(res.amount);
    res.blockNumber = utils.toBigNumber(res.blockNumber);

    if (res.payloadType === 3 || res.payloadType === 4) {
        res.payloadBody.amount = utils.toBigNumber(res.payloadBody.amount);
    }
    else if (res.payloadType === 5 || res.payloadType === 6) {
        res.payloadBody.amount = utils.toBigNumber(res.payloadBody.amount);
    }
    else if (res.payloadType === 7){
        res.payloadBody.rewardAmount = utils.toBigNumber(res.payloadBody.rewardAmount);

        res.payloadBody.minCommonTxFee = utils.toBigNumber(res.payloadBody.minCommonTxFee);
        res.payloadBody.minBondingTxFee = utils.toBigNumber(res.payloadBody.minBondingTxFee);
        res.payloadBody.minGRProposalTxFee = utils.toBigNumber(res.payloadBody.minGRProposalTxFee);
        res.payloadBody.minGRVoteTxFee = utils.toBigNumber(res.payloadBody.minGRVoteTxFee);
        res.payloadBody.minXTxFee = utils.toBigNumber(res.payloadBody.minXTxFee);

        res.payloadBody.validatorRewardRate = utils.toBigNumber(res.payloadBody.validatorRewardRate);
        res.payloadBody.delegatorRewardRate = utils.toBigNumber(res.payloadBody.delegatorRewardRate);

        res.payloadBody.maxBlockNumsForVoting = utils.toBigNumber(res.payloadBody.maxBlockNumsForVoting);
        res.payloadBody.minBlockNumsToGRProposal = utils.toBigNumber(res.payloadBody.minBlockNumsToGRProposal);
        res.payloadBody.minBlockNumsUtilReflection = utils.toBigNumber(res.payloadBody.minBlockNumsUtilReflection);
        res.payloadBody.maxBlockNumsUtilReflection = utils.toBigNumber(res.payloadBody.maxBlockNumsUtilReflection);

        res.payloadBody.blockNumsFreezingValidator = utils.toBigNumber(res.payloadBody.blockNumsFreezingValidator);
        res.payloadBody.blockNumsUtilUnbonded = utils.toBigNumber(res.payloadBody.blockNumsUtilUnbonded);
        res.payloadBody.maxDelegatableValidatorNums = utils.toBigNumber(res.payloadBody.maxDelegatableValidatorNums);
        res.payloadBody.validatorNums = utils.toBigNumber(res.payloadBody.validatorNums);

        res.payloadBody.currentReflection.blockNumsForVoting = utils.toBigNumber(res.payloadBody.currentReflection.blockNumsForVoting);
        res.payloadBody.currentReflection.blockNumsUtilReflection = utils.toBigNumber(res.payloadBody.currentReflection.blockNumsUtilReflection);
    }
    else if (res.payloadType === 10) {
        res.payloadBody.depth = utils.toBigNumber(res.payloadBody.depth);
        res.payloadBody.airdropRate = utils.toBigNumber(res.payloadBody.airdropRate);

        for (var i=0; i<res.payloadBody.assetHolders.length; i++) {
            res.payloadBody.assetHolders[i].amount = utils.toBigNumber(res.payloadBody.assetHolders[i].amount);
        }
        for (var i=0; i<res.payloadBody.seeds.length; i++){
            res.payloadBody.seeds[i].port = utils.toBigNumber(res.payloadBody.seeds[i].port);
        }
        for (var i=0; i<res.payloadBody.validators.length; i++){
            res.payloadBody.validators[i].power = utils.toBigNumber(res.payloadBody.validators[i].power);
        }
    }*/
...
function checkPayloadBody(pType, pBodyObj) {
    if (pType === 1) {
        ...
        objValueCheckStringNumber("payloadBody.rewardAmount", pBodyObj.rewardAmount);
        objValueCheckStringNumber("payloadBody.validatorRewardRate", pBodyObj.validatorRewardRate);
        objValueCheckStringNumber("payloadBody.delegatorRewardRate", pBodyObj.delegatorRewardRate);

        objValueCheckStringNumber("payloadBody.minCommonTxFee", pBodyObj.minCommonTxFee);
        objValueCheckStringNumber("payloadBody.minBondingTxFee", pBodyObj.minBondingTxFee);
        objValueCheckStringNumber("payloadBody.minGRProposalTxFee", pBodyObj.minGRProposalTxFee);
        objValueCheckStringNumber("payloadBody.minGRVoteTxFee", pBodyObj.minGRVoteTxFee);
        objValueCheckStringNumber("payloadBody.minXTxFee", pBodyObj.minXTxFee);

        objValueCheckStringNumber("payloadBody.maxBlockNumsForVoting", pBodyObj.maxBlockNumsForVoting);
        objValueCheckStringNumber("payloadBody.minBlockNumsToGRProposal", pBodyObj.minBlockNumsToGRProposal);
        objValueCheckStringNumber("payloadBody.minBlockNumsUtilReflection", pBodyObj.minBlockNumsUtilReflection);
        objValueCheckStringNumber("payloadBody.maxBlockNumsUtilReflection", pBodyObj.maxBlockNumsUtilReflection);

        objValueCheckStringNumber("payloadBody.blockNumsFreezingValidator", pBodyObj.blockNumsFreezingValidator);
        objValueCheckStringNumber("payloadBody.blockNumsUtilUnbonded", pBodyObj.blockNumsUtilUnbonded);
        objValueCheckStringNumber("payloadBody.maxDelegatableValidatorNums", pBodyObj.maxDelegatableValidatorNums);
        objValueCheckStringNumber("payloadBody.validatorNums", pBodyObj.validatorNums);

        objValueCheckString("payloadBody.firstCompatibleVersion", pBodyObj.firstCompatibleVersion);

        objValueCheckStringNumber("payloadBody.currentReflection.blockNumsForVoting", pBodyObj.currentReflection.blockNumsForVoting);
        objValueCheckStringNumber("payloadBody.currentReflection.blockNumsUtilReflection", pBodyObj.currentReflection.blockNumsUtilReflection);
